...
. Add the following firewalld task with *notify* at the end of *system_setup.yml* playbook. Remember to check the indentation!
+
[source,yaml]
----
    - name: Allow HTTP traffic on web servers
...
. Add the following *handler* at the end of *system_setup.yml* playbook. Remember to check the indentation!
+
[source,yaml]
----
  handlers:
...
[IMPORTANT]
====
.💡 The *system_setup.yml* Playbook output output should look like this:
[source,yaml]
----
---
